...
| AI-4 | **Cost & key controls** | Per-run token budget + call cap in config; the netKB/system-prompt prefix is prompt-cached to cut repeat cost. API key loaded from env/`config` (**never** committed — enforced by existing `.gitignore` + SECURITY.md), redacted from logs. A run reports its estimated token spend. |
