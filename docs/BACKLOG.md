...
| **NEW** | The **first `/get_logs` after every boot returns an error** | Found 2026-08-14 while running the #11 confirmation against the live device. `utils.py::serve_logs` spawns `subprocess.Popen(f"sudo tail -f .../data/logs/* > {log_file_path}", shell=True)` and then **immediately** `open()`s that path in the same call — the shell redirect has not created the file yet, so the first request answers `{"status":"error","message":"[Errno 2] No such file or directory: '/home/bjorn/Bjorn/data/logs/temp_log.txt'"}`. The second request works (file exists, empty), and it fills from there. Reproduced twice on 192.168.1.35. Once per boot, because `shared.py:292 delete_webconsolelog()` removes it at startup. Knock-on: `get_stats_snapshot`'s `log_version` token is `0` until something first hits `/get_logs`, which is *correct* (`_asset_mtime` returns 0 for an absent file) but makes the #11 token check look broken on a fresh boot. *Fix:* create the file before spawning the tail (`open(log_file_path, 'a').close()`), or serve `""` on ENOENT instead of an error body. |
